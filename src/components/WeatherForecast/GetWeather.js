...
function WeatherForecast(props) {
  ...
  const getData = () => {
    ...
    city.current.innerText = option.innerText

    let url = "https://api.open-meteo.com/v1/forecast?latitude="+latitude+"&longitude="+longitude+"&daily=weathercode,temperature_2m_max,rain_sum,snowfall_sum,windspeed_10m_max&timezone=Europe%2FBerlin&forecast_days=3"
    ...
    axios.get(url).then(response => {
      ...
      for (let i = 0; i < days.time.length; i++) {
        let date = new Date(days.time[i])
        date = date.toLocaleString("sk-SK", {dateStyle: "short"})
        ...
        data.push({
          ...
          windspeed_10m_max: days.windspeed_10m_max[i]
        })
      }
      onSubmitData(data)
    })
